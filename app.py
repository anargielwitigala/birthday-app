...
<style>
    ...
    * {
    ...
    .reveal-button {
        background: linear-gradient(135deg, #3b82f6 0%, #87ceeb 100%);
        color: white;
        padding: 15px 40px;
        font-size: 1.1em;
        border: none;
...
st.markdown("""
<audio autoplay style="width: 0; height: 0;">
...
# Navigation
tab1, tab2, tab3, tab4 = st.tabs(["💌 MESSAGE", "📸 GALLERY", "🎁 PHOTO REVEAL", "✨ MEMORIES"])
...
# ==================== TAB 3: PHOTO REVEAL ====================
with tab3:
    st.markdown('<div class="section-title">Photo Reveal</div>', unsafe_allow_html=True)
    
    st.markdown("""
    <p style='text-align: center; font-size: 1.2em; color: #e0f2fe; margin-bottom: 40px;'>
    Click the button below to reveal our memories one by one 💙
    </p>
    """, unsafe_allow_html=True)
    
    if len(image_paths) > 0:
        # Initialize session state for photo reveal
        ...
        with col2:
            if st.button("🎁 Reveal Next Memory", use_container_width=True, key="reveal_btn"):
                st.session_state.reveal_index += 1
                if st.session_state.reveal_index >= len(image_paths):
                    st.session_state.reveal_index = len(image_paths) - 1
        
        st.markdown("<br>", unsafe_allow_html=True)
        
        if st.session_state.reveal_index >= 0:
            col_img = st.columns([1])[0]
            with col_img:
                try:
                    img = Image.open(image_paths[st.session_state.reveal_index])
                    st.markdown("""
                    <div style='
                        border-radius: 15px;
                        overflow: hidden;
                    ...
                    progress = (st.session_state.reveal_index + 1) / len(image_paths)
                    st.progress(progress)
                    st.caption(f"Memory {st.session_state.reveal_index + 1} of {len(image_paths)}")
                except Exception as e:
                    st.error("Could not load image")
        else:
            st.markdown("""
            <div style='text-align: center; padding: 50px; color: #87ceeb;'>
                <p style='font-size: 1.3em;'>🎁 Click the button above to reveal our memories!</p>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown("""
